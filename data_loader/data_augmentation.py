...
def get_motion_blur_kernel(sz=7) -> np.ndarray:
    """
...
def random_warp_rev(image, resolution=64):
    assert image.shape == (256, 256, 6)
    res_scale = resolution // 64
    assert res_scale >= 1, f"Resolution should be >= 64. Received {resolution}."
    ...
    target_image = cv2.warpAffine(image, mat, (resolution, resolution))
...
def random_color_match(image, fns_all_trn_data):
    ...
    rnd_clr_space_to_xyz = np.random.choice([True, False])
    if rnd_clr_space_to_xyz:
...
def read_image(
    ...
    if random_transform_args is None:
    ...
    if isinstance(fn, bytes):
        ...
        fns_all_trn_data = [
            fn_all.decode("utf-8")
            for fn_all in fns_all_trn_data
        ]
        pass
    ...
    if use_bm_eyes:
        ...
        if bm_eyes is None:
            print(
                f"Failed reading binary mask {dir_bm_eyes}/{raw_fn}. "
                f"If this message keeps showing, please check for existence of binary masks folder "
                f"or disable eye-aware training in the configuration."
            )
            bm_eyes = np.zeros_like(image)
            pass
        bm_eyes = cv2.resize(bm_eyes, (256, 256)) / 255.0
        pass
    else:
        bm_eyes = np.zeros_like(image)
        pass
    ...
    warped_img, target_img = random_warp_rev(image, resolution)
