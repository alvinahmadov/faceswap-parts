...
def cyclic_loss(net_gen1, net_gen2, real1):
    fake2 = net_gen2(real1)[-1]  # fake2 ABGR
    fake2_alpha = Lambda(lambda x: x[:, :, :, :1])(fake2)
    fake2 = Lambda(lambda x: x[:, :, :, 1:])(fake2)  # fake2 BGR
    cyclic1 = net_gen1(fake2)[-1]  # cyclic1 ABGR
...
def adversarial_loss(net_disc, real, fake_abgr, distorted, gan_training="mixup_LSGAN", **weights):
    ...
    if gan_training == "mixup_LSGAN":
        ...
        pred_fake = net_disc(concatenate([fake, distorted]))
        pred_mixup = net_disc(mixup)
        loss_disc = calc_loss(pred_mixup, lam * K.ones_like(pred_mixup), "l2")
        loss_gen = weights['w_D'] * calc_loss(pred_fake, K.ones_like(pred_fake), "l2")
        mixup2 = lam * concatenate([real, distorted]) + (1 - lam) * concatenate([fake_bgr, distorted])
        pred_fake_bgr = net_disc(concatenate([fake_bgr, distorted]))
        pred_mixup2 = net_disc(mixup2)
        loss_disc += calc_loss(pred_mixup2, lam * K.ones_like(pred_mixup2), "l2")
        loss_gen += weights['w_D'] * calc_loss(pred_fake_bgr, K.ones_like(pred_fake_bgr), "l2")
    elif gan_training == "relativistic_avg_LSGAN":
        real_pred = net_disc(concatenate([real, distorted]))
        fake_pred = net_disc(concatenate([fake, distorted]))
        loss_disc = K.mean(K.square(real_pred - K.ones_like(fake_pred))) / 2
        loss_disc += K.mean(K.square(fake_pred - K.zeros_like(fake_pred))) / 2
        loss_gen = weights['w_D'] * K.mean(K.square(fake_pred - K.ones_like(fake_pred)))

        fake_pred2 = net_disc(concatenate([fake_bgr, distorted]))
        loss_disc += K.mean(K.square(real_pred - K.mean(fake_pred2, axis=0) - K.ones_like(fake_pred2))) / 2
        loss_disc += K.mean(K.square(fake_pred2 - K.mean(real_pred, axis=0) - K.zeros_like(fake_pred2))) / 2
        loss_gen += weights['w_D'] * K.mean(
...
def reconstruction_loss(real, fake_abgr, mask_eyes, model_outputs, **weights):
    ...
    loss_gen = 0
    loss_gen += weights['w_recon'] * calc_loss(fake_bgr, real, "l1")
    loss_gen += weights['w_eyes'] * K.mean(K.abs(mask_eyes * (fake_bgr - real)))
...
def edge_loss(real, fake_abgr, mask_eyes, **weights):
    ...
    loss_gen = 0
    loss_gen += weights['w_edge'] * calc_loss(first_order(fake_bgr, axis=1), first_order(real, axis=1), "l1")
    loss_gen += weights['w_edge'] * calc_loss(first_order(fake_bgr, axis=2), first_order(real, axis=2), "l1")
    ...
    loss_gen += weights['w_eyes'] * K.mean(
        K.abs(resized_mask_eyes * (first_order(fake_bgr, axis=1) - first_order(real, axis=1)))
    )
    loss_gen += weights['w_eyes'] * K.mean(
        K.abs(resized_mask_eyes * (first_order(fake_bgr, axis=2) - first_order(real, axis=2)))
    )
    return loss_gen
...
def perceptual_loss(real, fake_abgr, distorted, mask_eyes, vggface_feats, **weights):
    ...
    loss_gen += weights['w_pl'][0] * calc_loss(instnorm()(fake_feat7), instnorm()(real_feat7), "l2")
    loss_gen += weights['w_pl'][1] * calc_loss(instnorm()(fake_feat28), instnorm()(real_feat28), "l2")
    loss_gen += weights['w_pl'][2] * calc_loss(instnorm()(fake_feat55), instnorm()(real_feat55), "l2")
    loss_gen += weights['w_pl'][3] * calc_loss(instnorm()(fake_feat112), instnorm()(real_feat112), "l2")
