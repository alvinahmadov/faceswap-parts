...
def adversarial_loss(net_d, real, fake_abgr, distorted, gan_training="mixup_LSGAN", **weights):
    ...
    if gan_training == "mixup_LSGAN":
        ...
        # discriminator loss
        loss_discr = calc_loss(pred_mixup, lam * K.ones_like(pred_mixup), "l2")
        # generator loss
        ...
        loss_discr += calc_loss(pred_mixup2, lam * K.ones_like(pred_mixup2), "l2")
        ...
        loss_discr = K.mean(K.square(real_pred - K.ones_like(fake_pred))) / 2
        loss_discr += K.mean(K.square(fake_pred - K.zeros_like(fake_pred))) / 2
        loss_gen = weights['w_D'] * K.mean(K.square(fake_pred - K.ones_like(fake_pred)))

        fake_pred2 = net_d(concatenate([fake_bgr, distorted]))
        loss_discr += K.mean(K.square(real_pred - K.mean(fake_pred2, axis=0) - K.ones_like(fake_pred2))) / 2
        loss_discr += K.mean(K.square(fake_pred2 - K.mean(real_pred, axis=0) - K.zeros_like(fake_pred2))) / 2
        loss_gen += weights['w_D'] * K.mean(
            K.square(real_pred - K.mean(fake_pred2, axis=0) - K.zeros_like(fake_pred2))
        ) / 2
        loss_gen += weights['w_D'] * K.mean(
            K.square(fake_pred2 - K.mean(real_pred, axis=0) - K.ones_like(fake_pred2))
        ) / 2
    else:
        raise ValueError(f"Receive an unknown GAN training method: {gan_training}")
